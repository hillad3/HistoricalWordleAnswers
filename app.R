    ...
    nav_panel(
      ...
      column(
        ...
        tags$ul(
          ...
          tags$li("A logical OR relationship can be indicated with a pipe '|'. For example, '^F|F$' will match with both FORCE and GRIEF because the F can be at the start or end of the word (and there is no restriction for an F's presence or absence in between)."),
          ...
          tags$li("A 'one or more' relationship can be indicated with a '+'. For example, 'BE+' will match both MAYBE (with one E) and BEEFY (with two E's). If looking for one or more of the group, it would be written '(BE)+'."),
    ...
    nav_panel(
      ...
      column(8,
        ...
        p(
          ...
          tags$span(", which I use to scrape the Wordle answers on this website)."),
        ...
        p(
          ...
          tags$span("Data is scraped using {rvest}, but the trigger for this action is manual "),
          tags$span("(so feel free to text me or bug me on my Github if this list is out of date)."),
